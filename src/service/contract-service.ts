...
class ContractService {
	async createContract(from: string, to: string): Promise<Conference> {
		...
		const privateConferece = await ConferenceModel.findOne({
			$or: [
				{
					users: [from, to],
				},
	...
	async getFriendListFromId(id: string): Promise<(UserDto | void)[]> {
		...
		const friends: (UserDto | void)[] = await Promise.all(
			friendsId.map((friendId) => userService.findOneById(friendId)),
		);
	...
	async deleteFriend(
		...
		const deletedResult = await ContractModel.deleteOne({
			$or: [
				{
					from: userId,
					to: friendId,
